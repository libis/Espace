...
class Europeana_EuropeanaController extends Omeka_Controller_AbstractActionController
{
    ...
    private function getApiSearchParams()
    {
        ...
        //libis_start
        $queryStr = "";
        $queryFilter = $this->getParam('searchfilter');
        if(isset($queryFilter) && $queryFilter != 'All'){
            $queryStr = $queryFilter.':'.str_replace('+',' ',$this->getApiSearchQueryParam());
        }
        else{
            $queryStr = $this->getApiSearchQueryParam();
        }
        //libis_end

        return array(
            //'query' => $this->getApiSearchQueryParam(),
            'query' => $queryStr, //libis temp
            'rows'  => $perPage,
            'start' => $firstResult,
            'profile' => 'rich',
        );
